...
def parser_add_mf_options(parser):
    ...
    ## Ismael Kone, MF with labels
    group.add_option('--split_policy', dest='split_policy', nargs=2, default=(1,1), type='int', action='store',
            help='number of features randomly sampled and that of the split location. [default: %default meaning no label involved.] ')
    group.add_option('--criterion', dest='criterion', default='entropy', type='str', action='store',
            help='criterion used for splitting ("entropy" or "gini"). [default: %default] ')
...
def parser_check_mf_options(parser, settings):
    ...
    fail(parser, not(settings.criterion=='entropy' or settings.criterion=='gini') , 'criterion should be "entropy" or "gini".')
...
def load_data(settings):
    ...
    if not special_cases:
        data = pickle.load(open(settings.data_path + settings.dataset + '/' + \
                settings.dataset + '.p', "rb"), encoding='latin1')
    ...
    elif settings.dataset == 'acdc':
        data = load_acdc_data(settings.data_path + settings.dataset + '/')
    ...
    if is_mondrianforest:
        ...
        for idx_minibatch in range(n_minibatches):
            ...
            if is_last_minibatch:
                ...
                idx_tmp = np.arange(idx_minibatch * n_points_per_minibatch, data['n_train']).astype(int)
...
def load_acdc_data(path):
    x = np.load(path+'x_train.npy')
    y = np.load(path+'y_train.npy')
    x_train, y_train = x, y
    x_test = np.load(path+'x_test.npy')
    y_test = np.load(path+'y_test.npy')
    h_tr, w_tr, n_slices_tr = np.max(x_train, 1)
    h_te, w_te, n_slices_te = np.max(x_test, 1)
    data = {'x_train': x_train, 
            ...
            'is_sparse': False,
            'hwn_tr':(h_tr, w_tr, n_slices_tr), 
            'hwn_te':(h_te, w_te, n_slices_te)  
    }
    return data
...
## *** Ismael KONE
## Label guided splits implementation

def info_gain(H, split, criterion_func):
    (train_ids_left, train_ids_right, cache_tmp) = split
    nleft, nright = len(train_ids_left), len(train_ids_right)
    Hleft = criterion_func(cache_tmp['cnt_left_chosen'])
    Hright = criterion_func(cache_tmp['cnt_right_chosen'])
    IG = H - (nleft * Hleft + nright * Hright) /(nleft + nright)
    return IG

def split_node(node_id, data, param, cache, train_ids, settings):
    t_dim, t_loc = settings.split_policy
    criterion_funcs= {'entropy': compute_entropy, 'gini': compute_gini}
    criterion = settings.criterion
    best_g = -1.0
    best_split = None
    H = compute_entropy(cache['y_train_counts'])
    for d in range(t_dim):
        feat = sample_multinomial_scores(node_id.range_d)
        for l in range(t_loc):
            loc = random.uniform(node_id.min_d[feat], node_id.max_d[feat])
            split = compute_left_right_statistics(data, param, cache, train_ids, feat, loc, settings)
            g = info_gain(H, split, criterion_funcs[criterion])
            if best_g < g:
                best_g, best_split, best_feat, best_loc = g, split, feat, loc
    return best_split, best_feat, best_loc
...
def compute_entropy(cnts, alpha=0.0):
    ...
    for k in range(len(cnts)):
        if abs(prob[k]) > 1e-12:
            entropy -= prob[k] * np.log2(prob[k])
    return entropy

def compute_gini(cnts, alpha=0.0):
    prob = cnts * 1.0 + alpha / len(cnts)
    prob /= float(np.sum(prob))
